...
(function ($) {
  ...
  var toBeLoaded = [];
  $('[data-image-priority]').each(function(index) {
    $this = $(this);
    var src = $this.attr('src');
    toBeLoaded[index] = false;
    var $img = $('<img/>');
    $img.attr('src', src);
    $img.on('load', function() {
      toBeLoaded[index] = true;
      var every = true;
      for(var i=0;i<toBeLoaded.length;i++){
        if (toBeLoaded[i] === false) {
          every = false;
        };
      }
      if (every) {
        changeImgToBg('.js-bg-after-priority', undefined, 'data-src');
        $('.js-bg-after-priority').each(function() {
          $this = $(this);
          $this.attr('src', $this.attr('data-src'));
        });
      }
    ...
    metroIsotop();
      $('.loading').on('click', function () {
        ...
        function customDelay() {
            metroItem.find('.aheto-pf__item').removeClass('hide');
  ...
  if (blogItem.length) {
    $('.loading').on('click', function () {
      ...
      thisLink.addClass('load');
      function customDelay() {
        blogItem.find('.post').addClass('show');
    ...
    $('.load-more a').on('click', function () {
        $('.aheto-pf__item').removeClass('hide');
